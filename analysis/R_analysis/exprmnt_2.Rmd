...
#aqmod <- summary(aqm)
#saveRDS(aqmod,"/Users/bryangonzalez/KnowledgeBeliefASD/analysis/R_analysis/rtFiff_AQmod.rda")
...
_Agent State x Ascription within Groups_  
  In the neurotypical group, knowledge attributions (*M*~rt~=`r round(mean(df[df$group=="neurotypical" & df$ascription == "Knows" & df$agent_state =="Ignorance",]$rt), digits = 2)`ms, *SD*~rt~=`r round(sd(df[df$group=="neurotypical" & df$ascription == "Knows" & df$agent_state =="Ignorance",]$rt),digits = 2)`) were significantly faster than attributions of beliefs (*M*~rt~=`r round(mean(df[df$group=="neurotypical" & df$ascription == "Thinks" & df$agent_state =="Ignorance",]$rt), digits = 2)`ms, *SD*~rt~=`r round(sd(df[df$group=="neurotypical" & df$ascription == "Thinks" & df$agent_state =="Ignorance",]$rt),digits = 2)`) for agents that are ignorant, *t*(`r agntxascrpt_nt_post[agntxascrpt_nt_post$contrast == "Knows,Ignorance - Thinks,Ignorance",]$df`) = `r agntxascrpt_nt_post[agntxascrpt_nt_post$contrast == "Knows,Ignorance - Thinks,Ignorance",]$t.ratio`, *p* < `r format(max(agntxascrpt_nt_post[agntxascrpt_nt_post$contrast == "Knows,Ignorance - Thinks,Ignorance",]$p.value, .0001), scientific=F)`. Participants in the autism group did not show the same interaction effect, $\chi^2(2)$ = `r round(agntxascrpt_asd$Chisq[2],digits=3)`, *p* = `r round(agntxascrpt_asd$'Pr(>Chisq)'[2],digits=3)`.  
...
## Discussion
Provocative and impressive musings on implications of this study.